...
import { View, Text, StyleSheet, ScrollView, Image, RefreshControl,
    FlatList, TouchableOpacity, ActivityIndicator, Platform, Dimensions } from "react-native";
...
function Home() {
    ...
    const {setIsLogIn, setSingleTransaction, setInfo, setUser, isView, setIsView} = useContext(GlobalContext);
   ...
   useEffect(() => {
    ...
    }, 3000); // Auto slide every 3 second
    ...
    useEffect(() => {
        ...
        if (user) {
            ...
            .then( (res) => {
                    ...
                    setNotification(res?.data?.notifications)
                    setLoading(false);
                    if (res?.data?.user?.pin === null || res?.data?.user?.pin === "") {
                        setIsPinSet(false)
                        setTimeout( () => {
                            //setIsFocus(true)
    ...
    const handlePinCreation = (pin) => {
        ...
        .catch((err) => {
            ref?.current?.close()
            setLoadingPin(false)
            setPinResponse(err?.response?.data)
            })
        }
